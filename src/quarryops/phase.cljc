(ns quarryops.phase
  "Phase 0->3 staged rollout for the ISIC-0899 'Other mining and
  quarrying n.e.c.' SITE OPERATIONS-COORDINATION actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-extraction    -- extraction-record logging
                                        allowed, every write needs human
                                        approval.
    Phase 2  assisted-coordination  -- adds extraction-scheduling and
                                        shipment coordination writes,
                                        still approval.
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                        `:log-extraction-record`/
                                        `:schedule-extraction-operation`/
  ...
  fact, not a rollout milestone still to come. Surfacing an
  environmental concern always needs a human to actually look at it.
  `quarryops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [quarryops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-environmental-concern` is a member of
...
(def phases
  ...
  {0 {:label "read-only"             :writes #{}                                                      :auto #{}}
   ...
   2 {:label "assisted-coordination" :writes #{:log-extraction-record :schedule-extraction-operation
                                               :coordinate-shipment}                                   :auto #{}}
   3 {:label "supervised-auto"       :writes write-ops
      :auto #{:log-extraction-record :schedule-extraction-operation :coordinate-shipment}}})
...
(defn gate
  ...
  - `:flag-environmental-concern` is never auto-eligible at any phase,
...
(defn verdict->disposition
  "Map a QuarrySiteGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
