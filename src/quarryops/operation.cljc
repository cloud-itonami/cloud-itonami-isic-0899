(ns quarryops.operation
  ...
  (QuarryOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the QuarrySiteGovernor (:govern)
  ...
  human operator/site supervisor. The approver resumes with
...
(defn build
  ...
  opts:
    :advisor      -- a `quarryops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; QuarryOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; QuarrySiteGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
